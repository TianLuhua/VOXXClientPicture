...
import com.sat.satpic.IScanDevice;
...
public class ScanDevices {
    ...
    private static final String TAG = ScanDevices.class.getSimpleName();
    ...
    private ArrayList<String> mIpList = new ArrayList<String>();// ping成功的IP地址
        ...
        for (int i = 1; i < 255; i++) {// 创建256个线程分别去ping
            ...
            Runnable run = new Runnable() {
                ...
                public void run() {
                    ...
                    String ping = ScanDevices.this.mPing + mLocAddress
                            + lastAddress;
                    ...
                    try {
                     ...
                     //   Log.d(TAG, "hdb---run4:");
                        ...
                        if (result == 0) {
                            ...
                            if (iScanDevice != null) {
								iScanDevice.onScanSuccess(mIpList);
							}
                        } else {
                            // 扫描失败
                            Log.d(TAG, "扫描失败");
    ...
    private IScanDevice iScanDevice;
    public void setOnScanDevice(IScanDevice iScanDevice){
    	this.iScanDevice = iScanDevice;
    }
